...
abstract class AbstractStackableDrawerBlock(settings: Settings): AbstractDrawerBlock(settings) {
    init {
        defaultState = defaultState.with(ModProperties.STACKABLE_DRAWER_PART, StackableDrawerPart.SINGLE)
    }
    ...
    override fun getPlacementState(ctx: ItemPlacementContext): BlockState? {
        return super.getPlacementState(ctx)?.with(ModProperties.STACKABLE_DRAWER_PART, StackableDrawerPart.SINGLE)
    }
    ...
    override fun onPlaced(
        ...
        if (upState.block is AbstractStackableDrawerBlock) {
            ...
            val property = if (downState.block is AbstractStackableDrawerBlock) StackableDrawerPart.CENTER else StackableDrawerPart.DOWN
            val nextProperty = if (next.block is AbstractStackableDrawerBlock) StackableDrawerPart.CENTER else StackableDrawerPart.UP
            if (!upState.isAir) world.setBlockState(upPos, upState.with(ModProperties.STACKABLE_DRAWER_PART, nextProperty))
        ...
        if (downState.block is AbstractStackableDrawerBlock) {
            ...
            val property = if (upState.block is AbstractStackableDrawerBlock) StackableDrawerPart.CENTER else StackableDrawerPart.UP
            val downProp = if (next.block is AbstractStackableDrawerBlock) StackableDrawerPart.CENTER else StackableDrawerPart.DOWN
            if (!downState.isAir) world.setBlockState(downPos, downState.with(ModProperties.STACKABLE_DRAWER_PART, downProp))
            world.setBlockState(pos, state.with(ModProperties.STACKABLE_DRAWER_PART, property))
        }
    }
    ...
    override fun onBreak(world: World, pos: BlockPos, state: BlockState, player: PlayerEntity?): BlockState {
        if(!world.isClient) {
            ...
            if (upState.block is AbstractStackableDrawerBlock) {
                val nextProperty = if (world.getBlockState(nextUpPos).block is AbstractStackableDrawerBlock) StackableDrawerPart.DOWN else StackableDrawerPart.SINGLE
                if (!upState.isAir) world.setBlockState(upPos, upState.with(ModProperties.STACKABLE_DRAWER_PART, nextProperty))
            }
            if (downState.block is AbstractStackableDrawerBlock) {
                val downProp = if (world.getBlockState(nextDownPos).block is AbstractStackableDrawerBlock) StackableDrawerPart.UP else StackableDrawerPart.SINGLE
                if (!downState.isAir) world.setBlockState(downPos, downState.with(ModProperties.STACKABLE_DRAWER_PART, downProp))
            }
        }
    ...
    private fun updateNeighborhood(world: World, pos: BlockPos, state: BlockState) {
        if (world.isClient) return
        val (upPos, downPos) = MathUtils.getSideBlocksV(state, pos)
        val (nextUpPos, nextDownPos) = MathUtils.getNextSideBlocksV(state, pos)
