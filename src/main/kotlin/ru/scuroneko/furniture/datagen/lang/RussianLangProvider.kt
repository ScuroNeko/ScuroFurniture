...
class RussianLangProvider(output: FabricDataOutput, lookup: CompletableFuture<RegistryWrapper.WrapperLookup>) :
    FabricLanguageProvider(output, "ru_ru", lookup) {
    override fun generateTranslations(registryLookup: RegistryWrapper.WrapperLookup, builder: TranslationBuilder) {
        ...
        builder.add(Constants.Translatable.KITCHEN_CABINET, "Кухонный шкаф")
        builder.add(Constants.Translatable.SINGLE_DRAWER, "Одиночная тумба")
        ...
        RegistryHelper.Items.SMALL_DRAWERS_BOXES.forEach { box -> builder.add(box, "Ящик для медикаментов") }
        RegistryHelper.Items.MEDIUM_DRAWERS_BOXES.forEach { box -> builder.add(box, "Средний ящик") }
        RegistryHelper.Items.BIG_DRAWERS_BOXES.forEach { box -> builder.add(box, "Большой ящик") }

        RegistryHelper.Items.MEDIUM_DRAWERS_DOORS.forEach { door -> builder.add(door, "Дверца кухонного шкафчика") }
        RegistryHelper.Items.MEDIUM_DRAWERS_GLASS_DOORS.forEach { door ->
            builder.add(
                door,
                "Стеклянная дверца кухонного шкафчика"
            )
        }
        ...
        RegistryHelper.Blocks.KITCHEN_GLASS_CABINETS.forEach { drawer ->
            builder.add(
                drawer,
                "Стеклянный кухонный ящик"
            )
        }
